namespace Connector.Client
{
    ...
    public static class ClientHelper
    {
        public static class AuthTypeKeyEnums
        {
            ...
            public const string ConnectOAuth2CodeFlow = "connectOAuth2CodeFlow"; // New entry for Connect
        }
        ...
        public static void ResolveServices(this IServiceCollection serviceCollection, ConnectionContainer activeConnection)
        {
            ...
            switch (activeConnection.DefinitionKey)
            {
                case AuthTypeKeyEnums.OAuth2CodeFlow:
                    ...
                    serviceCollection.AddHttpClient<ApiClient, ApiClient>(client => new ApiClient(client, configOAuth2CodeFlow!.BaseUrl))
                        .AddHttpMessageHandler<OAuth2CodeFlowHandler>()
                        .AddHttpMessageHandler<RetryPolicyHandler>();
                    break;

                case AuthTypeKeyEnums.ConnectOAuth2CodeFlow: // New case for Connect
                    var configConnectOAuth2CodeFlow = JsonSerializer.Deserialize<OAuth2CodeFlow>(activeConnection.Configuration, options);
                    serviceCollection.AddSingleton<OAuth2CodeFlowBase>(configConnectOAuth2CodeFlow!);
                    serviceCollection.AddTransient<RetryPolicyHandler>();
                    serviceCollection.AddTransient<OAuth2CodeFlowHandler>();
                    serviceCollection.AddHttpClient<ApiClient, ApiClient>(client => new ApiClient(client, configConnectOAuth2CodeFlow!.BaseUrl))
                        .AddHttpMessageHandler<OAuth2CodeFlowHandler>()
                        .AddHttpMessageHandler<RetryPolicyHandler>();
                    break;
